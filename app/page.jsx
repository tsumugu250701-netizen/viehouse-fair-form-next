    ...
    <main className="page">
      ...
      <section className="giftLead">
        ...
        <div className="giftLeadMain">
          ...
          <div className="miniGiftList">
            {gifts.map((gift) => (
              <div className="miniGift" key={gift.value}>
                <img src={gift.image} alt="" />
                <strong>{gift.title.replace('カタログギフト','').replace('ギフト','')}</strong>
                <span>{gift.title.includes('カタログ') ? 'カタログギフト' : 'ギフト'}</span>
              </div>
            ))}
          </div>
      ...
      <section className="reasons">
        ...
        <div className="reasonGrid">
          <Reason no="01" icon={<HouseIcon />} title="デザイン・心地よさを体感">自然素材の温もりや、南仏・プロヴァンススタイルの心地よい空間を、実際に体感できます。</Reason>
          <Reason no="02" icon={<PerformanceIcon />} title="高性能な住まいを体感">断熱性・気密性など、性能にこだわった快適な住まいを、体感を通して確認いただけます。</Reason>
          <Reason no="03" icon={<HeartIcon />} title="わかりやすい家づくり相談">土地探しや資金計画など、家づくりの不安や疑問を、経験豊富なスタッフが丁寧にサポートします。</Reason>
        </div>
      </section>
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="choices radioChoices">
                  {['ぜひ見学したい', '話だけ聞いてみたい', '今回は情報収集のみ'].map((item) => (
                    <label className="choice" key={item}>
                      <input type="radio" name={ENTRY.modelhouse} value={item} checked={modelhouse === item} onChange={() => handleModelhouse(item)} />
                      <span>{item}</span>
                    </label>
                  ))}
                </div>
              ...
              <Question title="Q5. 5千円相当 選べるプレゼントをご希望の方はお選びください。">
                {!giftEnabled && <p className="giftNotice">※「ぜひ見学したい」を選択すると、プレゼントを選べます。</p>}
                {giftEnabled && (
                  <div className="giftCards">
                    {gifts.map((item) => (
                      <label className={`giftCard ${gift === item.value ? 'selected' : ''}`} key={item.value}>
                        <input
                          type="radio"
                          name={ENTRY.gift}
                          value={item.value}
      ...
      <footer className="bottomPanel">
        <div className="bottomText">
          <h3>モデルハウス見学のご予約も受付中！</h3>
          <p>ご希望の方は、スタッフまでお気軽にお声がけください。</p>
          <a href="#survey">モデルハウスの詳細はこちら</a>
        </div>
        <img src="/model-house.jpg" alt="Vie houseモデルハウス" />
        <div className="contactCard">
          <div className="logo white">Vie house<span>smart-off-grid-house</span></div>
...
function HouseIcon(){ return <svg viewBox="0 0 64 64" aria-hidden="true"><path d="M9 30 32 11l23 19"/><path d="M16 28v25h32V28"/><path d="M27 53V38h10v15"/></svg> }
function PerformanceIcon(){ return <svg viewBox="0 0 64 64" aria-hidden="true"><path d="M9 30 32 11l23 19"/><path d="M16 28v25h32V28"/><path d="M31 38V23"/><path d="M25 45a7 7 0 1 0 14 0 7 7 0 0 0-14 0Z"/><path d="M45 22c-8 0-13 5-13 13 8 0 13-5 13-13Z"/></svg> }
function HeartIcon(){ return <svg viewBox="0 0 64 64" aria-hidden="true"><path d="M20 38h11l9-9a7 7 0 0 1 10 10L37 52H16"/><path d="M16 34v21"/><path d="M32 23c-8-8-20 2-9 13l9 8 9-8c11-11-1-21-9-13Z"/></svg> }
